...
public class OsmoseCE49Extension extends ControllerExtension {
   private TrackManager mTrackManager;
   private ApplicationManager mApplicationManager;
   private ExternalMidiPreferences mExternalMidiPreference;

   protected OsmoseCE49Extension(final OsmoseCE49ExtensionDefinition definition, final ControllerHost host) {
   ...
   public void init() {
      ...
      mTrackManager = new TrackManager(host, surface, host.getMidiInPort(1), host.getMidiOutPort(1), true);
      mApplicationManager = new ApplicationManager(host, surface, host.getMidiInPort(1), host.getMidiOutPort(1));

      final NoteInput playNoteInput = host.getMidiInPort(0).createNoteInput("Osmose CE 49 play", "??????");
