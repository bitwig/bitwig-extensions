...
public class PrintToClipData {
    ...
    private static final double[] STEP_RES =
        {500.0, 333.3333, 250.0, 166.666666, 125.0, 83.3333333, 62.5, 41.6666666666};
    ...
    private static byte toHex(final int c1, final int c2) {
        return (byte) (((c1 < HEX_CHAR_NUM ? c1 - HEX_CHAR_0 : c1 - HEX_CHAR_A) << 4) | (c2 < HEX_CHAR_NUM ? c2
            - HEX_CHAR_0 : c2 - 87));
    }
    
    private static int beatTimePattern(final byte[] data) {
        return (data[PrintToClipData.DATA_OFFSET - 3] << 14) + (data[PrintToClipData.DATA_OFFSET - 2] << 7) + data[
            PrintToClipData.DATA_OFFSET - 1];
    }
