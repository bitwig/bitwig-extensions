...
public class VolumeSliderLayer extends TrackSliderLayer {
    ...
    public VolumeSliderLayer(final ViewCursorControl viewCursorControl, final ModifierStates modifierStates,
        ...
        final SysExHandler sysExHandler, final ControllerHost host, final LpProHwElements hwElements,
    ...
    private void bindMaster(final MasterTrack masterTrack, final TrackBank effectBank) {
        for (int i = 0; i < 7; i++) {
            ...
            final SliderBinding binding =
                new SliderBinding(mode.getCcNr(), track.volume(), sliders[i], i, midiProcessor);
            masterLayer.addBinding(binding);
            masterBindings.add(binding);
        }
        final SliderBinding binding =
            new SliderBinding(mode.getCcNr(), masterTrack.volume(), sliders[7], 7, midiProcessor);
        masterLayer.addBinding(binding);
        masterBindings.add(binding);
    }
    ...
    private void initSceneButtons_advanced(final LpProHwElements hwElements) {
        for (int i = 0; i < 7; i++) {
            final int index = i;
            final LabeledButton sceneButton = hwElements.getSceneLaunchButtons().get(index);
    ...
    protected void updateFaderState() {
        if (isActive()) {
            refreshTrackColors();
            modeHandler.setFaderBank(panelLayout == PanelLayout.VERTICAL ? 0 : 1, mode, tracksExistsColors);
            switch (volumeMode) {
                case 0:
