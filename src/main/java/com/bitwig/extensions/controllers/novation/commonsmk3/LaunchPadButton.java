...
public abstract class LaunchPadButton {
    ...
    public static final int TAP_INTERVAL = 200;
    ...
    private DoubleTapState waitingForDouble = DoubleTapState.INIT;
    
    private enum DoubleTapState {
        INIT, WAIT_FOR_DOUBLE, DOUBLE_OCCURRED
    }
    ...
    public void bindDoubleTapCombo(final Layer layer, final Runnable singleAction, final Runnable doubleAction,
        final BooleanSupplier doubleRequired) {
        layer.bind(
            hwButton, hwButton.pressedAction(), () -> handleDoubleTap(singleAction, doubleAction, doubleRequired));
    }
    
    private void handleDoubleTap(final Runnable singleAction, final Runnable doubleAction,
        final BooleanSupplier doubleRequired) {
        if (doubleRequired.getAsBoolean()) {
            if (waitingForDouble != DoubleTapState.WAIT_FOR_DOUBLE) {
                recordedDownTime = System.currentTimeMillis();
                midiProcessor.delayAction(TAP_INTERVAL + 5, () -> {
                    if (waitingForDouble == DoubleTapState.DOUBLE_OCCURRED) {
                        doubleAction.run();
                    } else {
                        singleAction.run();
                    }
                    waitingForDouble = DoubleTapState.INIT;
                });
                waitingForDouble = DoubleTapState.WAIT_FOR_DOUBLE;
            } else {
                final long clickTime = System.currentTimeMillis() - recordedDownTime;
                if (clickTime < TAP_INTERVAL) {
                    waitingForDouble = DoubleTapState.DOUBLE_OCCURRED;
                } else {
                    waitingForDouble = DoubleTapState.INIT;
                }
                recordedDownTime = System.currentTimeMillis();
    ...
    private void initiateTimed(final Runnable pressAction, final int delayTime) {
        currentTimer = new TimedDelayEvent(pressAction, delayTime);
        midiProcessor.queueEvent(currentTimer);
    }
