...
public class StringUtil {
    ...
    public static String toSingleNoteValue(final int note) {
        return NOTES[note % 12];
    }
    
    public static String toNoteValue(final int midiNote) {
        final String base = NOTES[midiNote % 12];
        final int octave = midiNote / 12 - 2;
        return String.format("%s %d", base, octave);
    }
    
    public static String toNoteValueTight(final int midiNote) {
        final String base = NOTES[midiNote % 12];
        final int octave = midiNote / 12 - 2;
        return String.format("%s%d", base, octave);
    }
