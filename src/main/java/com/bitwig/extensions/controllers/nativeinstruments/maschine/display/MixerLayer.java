...
public class MixerLayer extends DisplayLayer implements NameContainer {
   ...
   public MixerLayer(final MaschineExtension driver, final String name) {
      ...
      HardwareButton[] touchButtons = driver.getTouchButtons();
      ...
      bindLightState((BooleanSupplier) () -> currentParamLayer != sendLayer || sendLayer.canScrollForwards(), rightNav);
      for (int i = 0; i < knobs.length; i++) {
         final int index = i;
         final Track track = mixerBank.getItemAt(i);
         HardwareButton touchButton = touchButtons[i];
         ...
         volumeLayer.bindPressed(touchButton, () -> track.volume().touch(true));
         volumeLayer.bindReleased(touchButton, () -> track.volume().touch(false));
         ...
         panLayer.bindPressed(touchButton, () -> track.pan().touch(true));
         panLayer.bindReleased(touchButton, () -> track.pan().touch(false));
         ...
         sendLayer.bindPressed(touchButton, () -> sendItem.touch(true));
         sendLayer.bindReleased(touchButton, () -> sendItem.touch(false));
   ...
   public void incLayer(final int inc) {
      ...
      if (nextMode != null) {
         ...
         sendToDisplay(0, currentParamLayer.getTitle() + (currentParamLayer == sendLayer ? " " + currentSendName : ""));
         sendToDisplay(1, "");
      }
   }
