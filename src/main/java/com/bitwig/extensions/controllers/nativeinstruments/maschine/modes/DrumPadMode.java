...
public class DrumPadMode extends BasicKeyPlayingMode implements JogWheelDestination {
   ...
   public DrumPadMode(final MaschineExtension driver, final String name, final NoteFocusHandler noteFocusHandler,
      ...
      getDriver().getCursorTrack()
         .color()
         .addValueObserver((r, g, b) -> cursorTrackColor = NIColorUtil.convertColorX(r, g, b));
      ...
      for (int i = 0; i < buttons.length; i++) {
         ...
         pad.color().addValueObserver((r, g, b) -> padColors[index] = NIColorUtil.convertColorX(r, g, b));
         ...
         colorChooseLayer.bindPressed(button, () -> chooseColor(index));
      }
   }
   ...
   private void chooseColor(final int index) {
      final DrumPad pad = drumPadBank.getItemAt(index);
      if (pad.exists().get()) {
   ...
   public InternalHardwareLightState colorStateOfNoPads(final int index) {
      return RgbLed.of(cursorTrackColor + (playing[index] ? 2 : 0)); // TO this need to be done with lookup
      // table
   }

   private InternalHardwareLightState computeGridLedState(final int index, final DrumPad pad) {
      if (hasDrumPads) {
         int color = padColors[index];
         if (playing[index] || isSelected[index]) {
            color += 2;
         }
         return RgbLed.of(color); // TO this need to be done with lookup table
      } else {
         return colorStateOfNoPads(index);
      }
   }
