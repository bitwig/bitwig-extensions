...
public class EncoderLayer extends AbstractControlLayer {
   ...
   public void init(HardwareElementsApc hwElements, ViewControl viewControl) {
      ...
      for (int i = 0; i < 8; i++) {
         ...
         encoder.bindParameter(layerMap.get(ControlMode.VOLUME), track.volume());
         encoder.bindParameter(layerMap.get(ControlMode.PAN), track.pan());
         encoder.bind(layerMap.get(ControlMode.SEND), sendBank.getItemAt(0).value());
         Parameter parameter = parameterBank.getParameter(i);
         encoder.bindParameter(layerMap.get(ControlMode.DEVICE), parameter);
      }
   }
