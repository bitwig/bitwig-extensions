...
public class DrumPadLayer extends Layer {
    ...
    public DrumPadLayer(final MiniLab3Extension driver) {
        ...
        for (int i = 0; i < buttons.length; i++) {
            ...
            final MinilabRgbButton button = buttons[i];
            final int slotIndex = driver.getModel() == MinilabModel.MINILAB_3 ? index : (1 - i / 4) * 4 + i % 4;
            
            pad.color().addValueObserver((r, g, b) -> colorSlots[index] = RgbLightState.getColor(r, g, b));
            button.bindPressed(this, down -> handleSelected(index, down));
            button.bindLightState(this, () -> getLightState(slotIndex, pad));
        }
        ...
        final MultiStateHardwareLight bankLight =
            driver.getSurface().createMultiStateHardwareLight("PAD_LAUNCH_LIGHTS");
