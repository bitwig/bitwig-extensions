...
public class MiniLab3Extension extends ControllerExtension {
   ...
   protected MiniLab3Extension(final MiniLab3ExtensionDefinition definition, final ControllerHost host) {
    ...
    private void initCursors() {
        ...
        cursorTrack.name()
                .addValueObserver(
                        name -> updateTrackInfo(transport.isPlaying().get(), transport.isArrangerRecordEnabled().get(),
                                name, cursorDevice.name().get(), cursorDevice.exists().get()));
        cursorDevice.name()
                .addValueObserver(deviceName -> updateTrackInfo(transport.isPlaying().get(),
                        transport.isArrangerRecordEnabled().get(), cursorTrack.name().get(), deviceName,
                        cursorDevice.exists().get()));
        cursorDevice.exists()
                .addValueObserver(deviceExists -> updateTrackInfo(transport.isPlaying().get(),
                        transport.isArrangerRecordEnabled().get(), cursorTrack.name().get(), cursorDevice.name().get(),
                        deviceExists));
    ...
    private void setUpFollowArturiaDevice() {
        ...
        onArturiaDevice.addValueObserver(onArturia -> controlsAnalogLab.set(
                cursorTrack.arm().get() && cursorDevice.exists().get() && onArturia));
        cursorDevice.exists()
    ...
    private void setUpTransportControl() {
        ...
        transport.isArrangerLoopEnabled()
                .addValueObserver(loopEnabled -> oled.sendTextCond(DisplayMode.LOOP_VALUE, "Loop Mode",
                        loopEnabled ? "ON" : "OFF"));
    ...
    private void onMidi0(final ShortMidiMessage msg) {
        ...
        //getHost().println(String.format("MIDI %02x <%02d> %02x %02x", sb, channel, msg.getData1(), msg.getData2()));
    }
