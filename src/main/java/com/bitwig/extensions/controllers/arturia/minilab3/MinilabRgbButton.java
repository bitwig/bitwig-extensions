...
public class MinilabRgbButton {
    private final byte[] rgbCommand = {
    ...
    public MinilabRgbButton(final int padId, final PadBank bankId, final Type type, final int midiValue,
        final int channel, final MiniLab3Extension driver) {
        this.padId = padId;
        this.bankId = bankId;
        noteValue = midiValue;
        ...
        hwButton = driver.getSurface().createHardwareButton("RGB_PAD_" + midiValue);
        if (type == Type.NOTE) {
            hwButton.pressedAction().setActionMatcher(midiIn.createNoteOnActionMatcher(channel, midiValue));
            hwButton.releasedAction().setActionMatcher(midiIn.createNoteOffActionMatcher(channel, midiValue));
        } else {
            hwButton.pressedAction().setActionMatcher(midiIn.createCCActionMatcher(channel, midiValue, 127));
            hwButton.releasedAction().setActionMatcher(midiIn.createCCActionMatcher(channel, midiValue, 0));
        }
        hwButton.isPressed().markInterested();
        light = driver.getSurface().createMultiStateHardwareLight("RGB_PAD_LIGHT_" + midiValue);
    ...
    public void bindLightState(final Layer layer, final Supplier<RgbLightState> lightSource) {
    ...
    public void bind(final Layer layer, final Runnable action, final Supplier<RgbLightState> lightSource) {
        ...
        layer.bind(
            hwButton, hwButton.releasedAction(), () -> {
            });
        layer.bindLightState(lightSource::get, light);
    }
