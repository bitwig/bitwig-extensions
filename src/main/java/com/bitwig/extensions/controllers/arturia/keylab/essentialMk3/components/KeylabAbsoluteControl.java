...
public class KeylabAbsoluteControl {
    ...
    private final AbsoluteHardwareControl control;
    private final LcdDisplay.ValueType valueType;
    private final int index;
    private final int ccNr;
    private final MidiOut midiOut;
    
    public KeylabAbsoluteControl(final LcdDisplay.ValueType valueType, final int index, final HardwareSurface surface, final MidiIn midiIn,
    ...
    public void updateValue(final int currentValue) {
        if (valueType == LcdDisplay.ValueType.KNOB) {
            midiOut.sendSysex(String.format(SYS_EX_STATE, index + ENCODER_COMPONENT_ID, currentValue));
        }
    }
    
    public void forceValue(final boolean active) {
        if (!active) {
