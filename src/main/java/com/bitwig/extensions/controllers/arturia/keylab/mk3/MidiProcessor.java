...
public class MidiProcessor {
   ...
   public MidiProcessor(final ControllerHost host) {
      ...
      dawMainInput =
         midiIn.createNoteInput("MIDI", "89????", "99????", "B00B??", "A?????"); //"89????", "99????", "A9????"
   ...
   private void afterConnect() {
      screenLine2(
         ScreenTarget.POP_UP_TOP_ICON_2LINES,
         "Bitwig",
         RgbColor.WHITE,
         "Connected",
         RgbColor.WHITE,
   ...
   public void screenLine1(final ScreenTarget target, final String text, final RgbColor color, final CenterIcons icon) {
      final SysExBuilder sysEx = new SysExBuilder(target);
      sysEx.appendText(text);
      sysEx.appendValue(0); // color type  effects not clear
      sysEx.appendColor(color);
   ...
   public void screenLine2(final ScreenTarget target,
      final String text,
      final RgbColor color,
      final String text2,
   ...
   public void screenLine3(final ScreenTarget target,
      final String text,
      final RgbColor color,
      final String text2,
      final RgbColor color2,
      final String text3,
      final RgbColor color3,
      final CenterIcons icon) {
      final SysExBuilder sysEx = new SysExBuilder(target);
      sysEx.appendText(text);
      sysEx.appendValue(1);
   ...
   public void screenContextButton(final int contextId,
   ...
   public void submitControl(final int id, final boolean exists) {
      final String sysEx =
         "F0 00 20 6B 7F 42 00 %02X 00 %02X 00 %02X 00 01 %02X 00 02 %02X 00 F7".formatted(2, id, 1, 0, exists ? 1 : 0);
