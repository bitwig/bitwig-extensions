...
public class NoteSequenceLayer extends SequencerLayer {
    ...
    private void handleModifierChanged(final ModifierValueObject modifierValueObject) {
        if (copyNote != null) {
            ...
            getHeldNotes().stream().findFirst().map(noteStep -> assignments[noteStep.x()])
                .filter(NoteStepSlot::hasNotes).ifPresent(slot -> copyNote = slot.copy());
        }
        if (chordCopy != null) {
            if (!modifierValueObject.isDuplicateSet()) {
                chordCopy = null;
    ...
    private void initSelection(final MixerSectionHardware hwControls) {
        for (int i = 0; i < 8; i++) {
            ...
            hwControls.bindButton(recurrenceLayer, index, MixerSectionHardware.REC_INDEX,
                () -> maskLighting(mask, index), () -> editMask(mask));
    ...
    public boolean stepState(final int step) {
        ...
        if (hasStep) {
            if (copyNote != null && assignments[step] != null
                && assignments[step].getSlotNr() == copyNote.getSlotNr()) {
                return blinkTicks % 2 == 1;
            }
    ...
    private void handleTranspose(final List<NoteStep> notes, final int increment) {
        ...
        if (notes.size() == 1) {
            final int destNote =
                calcTranspose(notes.get(0).y(), mode == TransposeMode.OCTAVE ? increment * 12 : increment,
