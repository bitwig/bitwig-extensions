...
public class Context {
   ...
   private void tryToCompleteIncompleteSetter(final Class<?> newType) {
      ...
      while (it.hasNext()) {
         final ComponentClosure<?> closure = it.next();
         if (closure.instance != null && closure.missingSetters.remove(newType) && closure.missingSetters.isEmpty()) {
            try {
   ...
   private void injectBySetter(final Object object,
      ...
      for (final Method method : methods) {
         ...
         if (injectNotation != null && !Modifier.isPrivate(method.getModifiers()) && method.getParameterCount() == 1) {
